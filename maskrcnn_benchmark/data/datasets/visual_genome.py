...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix = get_VG_statistics(img_dir=self.img_dir, roidb_file=self.roidb_file, dict_file=self.dict_file,image_file=self.image_file, must_overlap=True)
        eps = 1e-3
        bg_matrix += 1
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    for i in range(len(image_index)):
        ...
        if split == 'train':
            ...
            for rel_i in rels:
                tri_rel_i = str(gt_classes_i[rel_i[0]]) + '-' + str(gt_classes_i[rel_i[1]]) + '-' + str(rel_i[2])
                if tri_rel_i not in tri_rel_list:
                    rel_i_t = rel_i
                    if rel_i[0] not in boxes_list:
                        boxes_list.append(rel_i[0])
                    if rel_i[1] not in boxes_list:
                        boxes_list.append(rel_i[1])
                    tri_rel_list.append(tri_rel_i)
                    rel_i_t[0] = np.where(np.array(boxes_list) == rel_i[0])[0][0]
                    rel_i_t[1] = np.where(np.array(boxes_list) == rel_i[1])[0][0]
                    rels_list.append(rel_i_t)
            if len(rels_list) == 0:
                split_mask[image_index[i]] = False
                continue
            rels = np.array(rels_list)
            boxes_ind = np.array(boxes_list)
            boxes.append(boxes_i[boxes_ind])
            gt_classes.append(gt_classes_i[boxes_ind])
            # print('after: ')
            # for rel_i in rels:
            #     print( gt_classes[-1][rel_i[0]], '-', gt_classes[-1][rel_i[1]], '-', rel_i[2])
            #     print( boxes[-1][rel_i[0]], '-', boxes[-1][rel_i[1]], '-', rel_i[2])
